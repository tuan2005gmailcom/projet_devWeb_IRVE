# -*- coding: utf-8 -*-
"""
Web/API compatible version of clusters_map.py.

It keeps the same prediction logic:
    cluster_model.pkl + scaler.pkl -> model.predict()

It can:
    - predict one or several coordinates
    - return JSON for PHP / JavaScript
    - optionally create a Folium HTML map

Examples:
    python clusters_map_web.py --lat 48.8566 --lon 2.3522 --json
    python clusters_map_web.py --coords "48.8566,2.3522;45.7640,4.8357" --json
    python clusters_map_web.py --lat 48.8566 --lon 2.3522 --output cluster_map.html --json
"""
...
def normalize_input_dataframe(df):
    ...
    df["consolidated_latitude"] = pd.to_numeric(
        df["consolidated_latitude"], errors="coerce"
    )
    df["consolidated_longitude"] = pd.to_numeric(
        df["consolidated_longitude"], errors="coerce"
    )
...
def parse_coordinates(args):
    ...
    if args.lat is not None or args.lon is not None:
        ...
        rows.append({
            "consolidated_latitude": args.lat,
            "consolidated_longitude": args.lon
        })
...
def create_cluster_map(result_df, output_html):
    ...
    if folium is None:
        raise ImportError("folium is not installed. Install it with: pip install folium")
...
def result_to_json(result_df, output_path=None):
    """Return API-friendly JSON data."""
    points = []
...
def print_results(result_df, output_path=None):
...
def main():
    ...
    parser.add_argument(
        ...
        action="append",
        metavar=("LAT", "LON"),
        help="Ajouter un point. Peut être répété plusieurs fois.",
    )
    parser.add_argument(
        "--coords",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Nom du fichier HTML de sortie. Exemple: cluster_map.html",
    )
    parser.add_argument(
        "--json",
        action="store_true",
        help="Retourner le résultat au format JSON pour PHP/JavaScript.",
    )
    ...
    try:
        ...
        if args.json:
            print(json.dumps(result_to_json(result_df, output_path), ensure_ascii=False))
        else:
